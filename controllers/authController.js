...
const generateTokenAndLink = (userId, purpose = 'activate') => {
    ...
    return `http://localhost:/api/auth/activate?token=${token}`;
};
...
class AuthController {
    async login(req, res) {
        try {
            ...
            console.log('[POST /api/auth/login] Attempting login:', { emailOrUsername, rememberMe });
    
            if (!emailOrUsername || !password) {
                console.log('[POST /api/auth/login] Missing email/username or password');
            ...
            if (!user) {
                console.log('[POST /api/auth/login] User not found:', emailOrUsername);
                return res.status(400).json({ message: 'Пользователь не найден' });
            }
    
            const validPassword = bcrypt.compareSync(password, user.password);
            if (!validPassword) {
                console.log('[POST /api/auth/login] Invalid password for:', emailOrUsername);
                return res.status(400).json({ message: 'Неверный пароль' });
            }
    
            if (!user.isActive) {
                console.log('[POST /api/auth/login] Account not activated:', emailOrUsername);
    ...
    async googleLogin(req, res) {
        try {
          const { token } = req.body;
          console.log('[POST /api/auth/google-login] Attempting Google login with token');
          
          if (!token) {
            console.log('[POST /api/auth/google-login] No token provided');
          ...
          console.log('[POST /api/auth/google-login] Google token verified:', { email, name });
          ...
          if (!user) {
            console.log('[POST /api/auth/google-login] User not found, creating new user:', email);
            
            // Get or create user role
            let userRole = await prisma.roles.findUnique({ where: { value: 'user' } });
            if (!userRole) {
              userRole = await prisma.roles.create({ data: { value: 'user' } });
              console.log('[POST /api/auth/google-login] Created role:', userRole);
            }
            
            // Generate a random username based on email if name is not provided
            ...
            user = await prisma.users.create({
              data: {
                username,
                email,
                password: await bcrypt.hash(Math.random().toString(36).slice(-10), 7), // Random password
                isActive: true, // Google accounts are already verified
            ...
            console.log('[POST /api/auth/google-login] New user created:', { userId: user.id, email });
          }
          
          // User exists but check if they're active
          if (!user.isActive) {
            console.log('[POST /api/auth/google-login] User exists but not active, activating:', email);
          ...
          // Generate JWT token
          const roles = user.user_roles.map(ur => ur.roles.value);
          const jwtToken = generateAccessToken(user.id, roles, true); // Use rememberMe for Google logins
          ...
          console.log('[POST /api/auth/google-login] Login successful, cookie set:', {
            tokenLength: jwtToken.length,
            userId: user.id,
          });
          
          return res.json({ message: 'Вход через Google выполнен успешно', userId: user.id });
        } catch (error) {
          console.error('[POST /api/auth/google-login] Error:', error.message, error.stack);
