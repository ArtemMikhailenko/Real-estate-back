    ...
    async createPost(req, res) {
        try {
            ...
            const response = {
                ...
                imageUrls: Array.isArray(post.imageUrls) ? post.imageUrls.map(url => `http://localhost:5001${url}`) : [],
                ...
                seller: {
                    ...
                    avatar: post.author.profilePicture ? `http://localhost:5001${post.author.profilePicture}` : null,
      ...
      try {
        ...
        const formattedAgents = agents.map(agent => {
          const profilePicture = agent.profilePicture
            ? `http://localhost:5001/uploads/${agent.profilePicture}`
            : null;
      ...
      try {
        ...
        const formattedProperties = properties.map(property => {
          const imageUrls = property.imageUrls.map(url => {
            ...
            if (url.startsWith('/Uploads/posts/')) return `http://localhost:5001${url}`;
            return `http://localhost:5001/uploads/posts/${url}`;
          });
          const authorProfilePicture = property.author.profilePicture
            ? `http://localhost:5001/uploads/profiles/${property.author.profilePicture}`
            : null;
      ...
      try {
        ...
        const formattedPosts = posts.map(post => {
          ...
          const authorProfilePicture = post.author.profilePicture
            ? `http://localhost:5001/uploads/${post.author.profilePicture}`
            : null;
